...
class Predictor:
    def __init__(self, model_identifier, num_movie_ids):
        ...
        if os.path.exists(model_identifier):
            self.model = load_model_from_filesystem(model_identifier, num_movie_ids)
        else:
            self.model = load_model_from_huggingface(model_identifier, num_movie_ids)
        self.model.eval()
    ...
    def predict(self, movie_id, n):
        ...
        # remove the input movie ID from the recommendations
